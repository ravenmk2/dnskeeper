...
| service | internal/service | Txn 组装、record-id 分配、嵌套冲突预检、级联删除、reconcile、登录/改密、密码强度            | 不直接碰 etcd API |
...
store 层共享 `*clientv3.Client`（构造器注入），封装 `Get`/`Put`/`Delete`/`WithPrefix`/`Txn` 与 JSON 序列化，key 前缀常量（`/dnskeeper/`、`/skydns/`）集中定义。Txn 组合规则与 reconcile 算法详见 [dns-sync.md](design/dns-sync.md) §3–§4；service 层随 Record CRUD 在同一 Txn 内执行单 Domain 级 reconcile，Domain/Zone 级移除按范围删同 Txn。集合级同步（含悬空清理）为预留扩展。
...
- **集成测试**：`etcd/embed` 在 `TestMain` 启进程内 etcd，覆盖 store/Txn/级联/reconcile，CI 无 Docker 依赖。
